...
show_command()
{
	...
	break;
    case S_DEBUG:
	fprintf(_stderr,"debug level is %d\n",debug);
	break;
    case S_DGRID3D:
	show_dgrid3d();
	break;
...
#define CHECK_TAG_GT_ZERO					\
	...
	break;
    case S_MONOCHROME:
	fprintf(_stderr,"monochrome mode is %s\n", monochrome ? "active" : "not active");
    ...
    case S_XYPLANE:
	if (xyplane.absolute)
	    fprintf(_stderr, "\txyplane intercepts z axis at %g\n", xyplane.z);
	else
	    fprintf(_stderr, "\txyplane %g\n", xyplane.z);
	break;
    case S_TIMESTAMP:
	show_timestamp();
	break;
...
/* FIXME: get rid of S_*DTICS, S_*MTICS cases */
    ...
    case S_MULTIPLOT:
	fprintf(_stderr,"multiplot mode is %s\n", multiplot ? "on" : "off");
	break;

    case S_TERMOPTIONS:
	fprintf(_stderr,"Terminal options are '%s'\n",
		(*term_options) ? term_options : "[none]");
	break;

    case S_THETA:
	fprintf(_stderr,"Theta increases %s with origin at %s of plot\n",
...
disp_at(struct at_type *curr_at, int level)
{
	...
	(void) putc('\t', _stderr);
	for (j = 0; j < level; j++)
	    (void) putc(' ', _stderr);   /* indent */

	/* print name of instruction */

	fputs(ft[(int) (curr_at->actions[i].index)].f_name, _stderr);
	...
	case PUSHC:
	    (void) putc(' ', _stderr);
	    disp_value(_stderr, &(arg->v_arg), TRUE);
	    (void) putc('\n', _stderr);
	    break;
	case PUSHD1:
	    fprintf(_stderr, " %c dummy\n",
	...
	case PUSHD2:
	    ...
	    break;
	case CALL:
	    fprintf(_stderr, " %s", arg->udf_arg->udf_name);
	    if (level < 6) {
		if (arg->udf_arg->at) {
		    (void) putc('\n', _stderr);
...
");
	...
	c_token++;
	fprintf(_stderr, "\nCompile options:\n%s", compile_options);
	fprintf(_stderr, "    %d-bit integer arithmetic\n\n",(int)sizeof(intgr_t)*8);
...
#ifdef X11
	{
	    ...
	    fprintf(_stderr, "GNUPLOT_DRIVER_DIR = \"%s\"\n", driverdir);
	}
#endif
...
#endif
	    if (psdir != NULL)
		fprintf(_stderr, "GNUPLOT_PS_DIR     = \"%s\"\n", psdir);
	}
...
# else
		helpfile = HELPFILE;
# endif
	fprintf(_stderr, "HELPFILE           = \"%s\"\n", helpfile);
#else /* _WIN32 */
	    fprintf(_stderr, "HELPFILE           = \"" TCHARFMT "\"\n", winhelpname);
#endif
	}

#if defined(_WIN32) && !defined(WGP_CONSOLE)
	fprintf(_stderr, "MENUNAME           = \"" TCHARFMT "\"\n", szMenuName);
#endif

#ifdef HAVE_LIBCACA
	fprintf(_stderr, "libcaca version    : %s\n", caca_get_version());
#endif
...
show_border()
{
    ...
    if (!draw_border)
	fprintf(_stderr, "\tborder is not drawn\n");
    else {
	fprintf(_stderr, "\tborder %d (0x%X) is drawn in %s layer with\n\t ",
	    draw_border, draw_border,
	    border_layer == LAYER_BEHIND ? "behind" : border_layer == LAYER_BACK ? "back" : "front");
	save_linetype(_stderr, &border_lp, FALSE);
	fputc('\n',_stderr);
    }
}
...
show_boxwidth()
{
	...
	fprintf(_stderr, "\tboxwidth is %g %s\n", boxwidth,
		(boxwidth_is_absolute) ? "absolute" : "relative");
    }
    fprintf(_stderr, "\tboxdepth is %g\n", boxdepth);
}
...
show_boxplot()
{
    fprintf(_stderr, "\tboxplot representation is %s\n",
	    boxplot_opts.plotstyle == FINANCEBARS ? "finance bar" : "box and whisker");
    fprintf(_stderr, "\tboxplot range extends from the ");
    if (boxplot_opts.limit_type == 1)
	fprintf(_stderr, "  median to include %5.2f of the points\n",
		boxplot_opts.limit_value);
    else
	fprintf(_stderr, "  box by %5.2f of the interquartile distance\n",
		boxplot_opts.limit_value);
    if (boxplot_opts.outliers)
	fprintf(_stderr, "\toutliers will be drawn using point type %d\n",
		boxplot_opts.pointtype+1);
    else
	fprintf(_stderr,"\toutliers will not be drawn\n");
    fprintf(_stderr,"\tseparation between boxplots is %g\n",
		boxplot_opts.separation);
    fprintf(_stderr,"\tfactor labels %s\n",
		...
		"are off");
    fprintf(_stderr,"\tfactor labels will %s\n",
...
show_fillstyle()
{
    ...
    case FS_TRANSPARENT_PATTERN:
        ...
        break;
    default:
        fprintf(_stderr, "\tFill style is empty");
    }
    if (default_fillstyle.border_color.type == TC_LT && default_fillstyle.border_color.lt == LT_NODRAW)
	fprintf(_stderr," with no border\n");
    else {
	fprintf(_stderr," with border ");
	save_pm3dcolor(_stderr, &default_fillstyle.border_color);
	fprintf(_stderr,"\n");
    }
}
...
show_clip()
{
    ...
    fprintf(_stderr, "\tpoint clip is %s\n", (clip_points) ? "ON" : "OFF");

    fprintf(_stderr, "\t%s lines with one end out of range (clip one)\n",
	clip_lines1 ? "clipping" : "not drawing");

    fprintf(_stderr, "\t%s lines with both ends out of range (clip two)\n",
	clip_lines2 ? "clipping" : "not drawing");

    fprintf(_stderr, "\t%sclipping lines on polar plot at maximum radius\n",
	clip_radial ? "" : "not ");
}
...
show_contour()
{
    ...
    fprintf(_stderr, "\tcontour for surfaces are %s",
	    (draw_contour) ? "drawn" : "not drawn\n");

    if (draw_contour) {
	fprintf(_stderr, " in %d levels on ", contour_levels);
	switch (draw_contour) {
	case CONTOUR_BASE:
	    fputs("grid base\n", _stderr);
	    break;
	...
	case CONTOUR_KIND_CUBIC_SPL:
	    fprintf(_stderr, "\t\tas cubic spline interpolation segments with %d pts\n", contour_pts);
	    break;
	case CONTOUR_KIND_BSPLINE:
	    fprintf(_stderr, "\t\tas bspline approximation segments of order %d with %d pts\n", contour_order, contour_pts);
	    break;
	}
	switch (contour_levels_kind) {
	case LEVELS_AUTO:
	    fprintf(_stderr, "\t\tapprox. %d automatic levels\n", contour_levels);
	    break;
	case LEVELS_DISCRETE:
	    {
		int i;
		fprintf(_stderr, "\t\t%d discrete levels at ", contour_levels);
		fprintf(_stderr, "%g", contour_levels_list[0]);
		for (i = 1; i < contour_levels; i++)
		    fprintf(_stderr, ",%g ", contour_levels_list[i]);
		putc('\n', _stderr);
		break;
	    }
	case LEVELS_INCREMENTAL:
	    fprintf(_stderr, "\t\t%d incremental levels starting at %g, step %g, end %g\n", contour_levels, contour_levels_list[0],
	...
	fprintf(_stderr, "\tcontour lines are drawn in %s linetypes\n",
		clabel_onecolor ? "the same" : "individual");
	fprintf(_stderr, "\tformat for contour labels is '%s' font '%s'\n",
		contour_format, clabel_font ? clabel_font : "");
	fprintf(_stderr, "\ton-plot labels placed at segment %d with interval %d\n",
		clabel_start, clabel_interval);
	if (contour_firstlinetype > 0)
		fprintf(_stderr, "\tfirst contour linetype will be %d\n", contour_firstlinetype);
	else
		fprintf(_stderr, "\tfirst contour linetype will be chosen automatically\n");
	fprintf(_stderr, "\tcontour levels will be %ssorted\n", contour_sortlevels ? "" : "un");
    }
}
...
show_dashtype(int tag)
{
	...
	if (tag == 0 || tag == this_dashtype->tag) {
	    showed = TRUE;
	    fprintf(_stderr, "\tdashtype %d, ", this_dashtype->tag);
	    save_dashtype(_stderr, this_dashtype->d_type, &(this_dashtype->dashtype));
	    fputc('\n', _stderr);
	}
    }
    if (tag > 0 && !showed)
...
show_mapping()
{
    ...
    fputs("\tmapping for 3-d data is ", _stderr);
...
show_dummy()
{
    ...
    fputs("\tdummy variables are ", _stderr);
    for (i=0; i<MAX_NUM_VAR; i++) {
	if (*set_dummy_var[i] == '\0') {
	    fputs("\n", _stderr);
	    break;
	} else {
	    fprintf(_stderr, "%s ", set_dummy_var[i]);
	}
    }
}
...
show_format()
{
    ...
    fprintf(_stderr, "\ttic format is:\n");
    save_axis_format(_stderr, FIRST_X_AXIS );
    save_axis_format(_stderr, FIRST_Y_AXIS );
    save_axis_format(_stderr, SECOND_X_AXIS);
    save_axis_format(_stderr, SECOND_Y_AXIS);
    save_axis_format(_stderr, FIRST_Z_AXIS );
    save_axis_format(_stderr, COLOR_AXIS);
    save_axis_format(_stderr, POLAR_AXIS);
}
...
#define CHECK_TAG_GT_ZERO					\
	...
	save_style_parallel(_stderr);
	c_token++;
	break;
    case SHOW_STYLE_SPIDERPLOT:
	save_style_spider(_stderr);
	c_token++;
	break;
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tRectangle style is %s, fill color ",
		default_rectangle.layer > 0 ? "front" :
		default_rectangle.layer < 0 ? "behind" : "back");
    save_pm3dcolor(_stderr, &default_rectangle.lp_properties.pm3d_color);
    fprintf(_stderr, ", lw %.1f ", default_rectangle.lp_properties.l_width);
    fprintf(_stderr, ", fillstyle");
    save_fillstyle(_stderr, &default_rectangle.fillstyle);
}
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tCircle style has default radius ");
    show_position(&default_circle.o.circle.extent, 1);
    fprintf(_stderr, " [%s]", default_circle.o.circle.wedge ? "wedge" : "nowedge");
    fputs("\n", _stderr);
}
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tEllipse style has default size ");
    show_position(&default_ellipse.o.ellipse.extent, 2);
    fprintf(_stderr, ", default angle is %.1f degrees", default_ellipse.o.ellipse.orientation);
...
{
    fprintf(_stderr, "\t%s are plotted with ", name);
    save_data_func_style(_stderr, name, style);
}
...
show_functions()
{
	...
	if (udf->definition)
	    ...
	    fprintf(_stderr, "\t%s is undefined\n", udf->udf_name);
	udf = udf->next_udf;
    }
}
...
#undef SHOW_GRID
    ...
    fprintf(_stderr, "\tMajor grid drawn with");
    save_linetype(_stderr, &(grid_lp), FALSE);
    fprintf(_stderr, "\n\tMinor grid drawn with");
    save_linetype(_stderr, &(mgrid_lp), FALSE);
    fputc('\n', _stderr);
    if (grid_vertical_lines)
	fprintf(_stderr, "\tVertical grid lines in 3D plots\n");
    if (polar_grid_angle)
	fprintf(_stderr, "\tGrid radii drawn every %f %s\n",
		polar_grid_angle / ang2rad,
		(ang2rad == 1.0) ? "radians" : "degrees");
    if (grid_spiderweb)
	fprintf(_stderr, "\tGrid shown in spiderplots\n");

    fprintf(_stderr, "\tGrid drawn at %s\n", (grid_layer==-1) ? "default layer" : ((grid_layer==0) ? "back" : "front"));
}

static void
show_raxis()
{
    fprintf(_stderr,"\traxis is %sdrawn\n",raxis ? "" : "not ");
}
...
show_paxis()
{
	...
	save_prange(_stderr, paxis);
    if (END_OF_COMMAND || almost_equals(c_token, "tic$s"))
	show_ticdefp(paxis);
    if (END_OF_COMMAND || equals(c_token, "label")) {
	fprintf(_stderr, "\t");
	save_axis_label_or_title(_stderr, axis_name(paxis->index),"label",
				&paxis->label, TRUE);
    }
    if (paxis->zeroaxis)
	save_linetype(_stderr, paxis->zeroaxis, FALSE);
    c_token++;
}
...
show_zeroaxis(AXIS_INDEX axis)
{
	...
	fprintf(_stderr, "\t%szeroaxis is drawn with", axis_name(axis));
	save_linetype(_stderr, axis_array[axis].zeroaxis, FALSE);
	fputc('\n',_stderr);
    } else
	fprintf(_stderr, "\t%szeroaxis is OFF\n", axis_name(axis));
}
...
show_label(int tag)
{
	...
	if (tag == 0 || tag == this_label->tag) {
	    ...
	    if (this_label->rotate)
	    	fprintf(_stderr, " rotated by %d degrees (if possible)", this_label->rotate);
	    else
	    	fprintf(_stderr, " not rotated");
	    fprintf(_stderr, " %s ", this_label->layer ? "front" : "back");
	    if (this_label->font != NULL)
		fprintf(_stderr, " font \"%s\"", this_label->font);
	    if (this_label->textcolor.type)
		save_textcolor(_stderr, &this_label->textcolor);
	    if (this_label->noenhanced)
		fprintf(_stderr, " noenhanced");
	    if ((this_label->lp_properties.flags & LP_SHOW_POINTS) == 0)
		fprintf(_stderr, " nopoint");
	    else {
		fprintf(_stderr, " point with color of");
		save_linetype(_stderr, &(this_label->lp_properties), TRUE);
		fprintf(_stderr, " offset ");
		show_position(&this_label->offset, 3);
	    }

	    if (this_label->boxed) {
		fprintf(_stderr," boxed");
		if (this_label->boxed > 0)
		    fprintf(_stderr," bs %d",this_label->boxed);
	    }
...
show_arrow(int tag)
{
	...
	if (tag == 0 || tag == this_arrow->tag) {
	    ...
	    save_linetype(_stderr, &(this_arrow->arrow_properties.lp_properties), FALSE);
	    fprintf(_stderr, "\n\t  from ");
		...
		fputs(" length ", _stderr);
		show_position(&this_arrow->end, 1);
		fprintf(_stderr," angle %g deg",this_arrow->angle);
	    }
		...
		if (this_arrow->arrow_properties.headfill != AS_NOFILL)
		    fprintf(_stderr,", backangle %g deg",
...
show_key()
{
	...
	fputs("\tkey is ON, position: ", _stderr);
	if (!(key->region == GPKEY_AUTO_EXTERIOR_MARGIN && (key->margin == GPKEY_TMARGIN || key->margin == GPKEY_BMARGIN))) {
		...
		fputs("bottom", _stderr);
	    else
		fputs("center", _stderr);
	}
	...
	if (key->stack_dir == GPKEY_VERTICAL) {
	    fputs(" vertical", _stderr);
	} else {
	    fputs(" horizontal", _stderr);
	}
	if (key->region == GPKEY_AUTO_INTERIOR_LRTBC)
	    fputs(key->fixed ? " fixed" : " inside", _stderr);
	else if (key->region == GPKEY_AUTO_EXTERIOR_LRTBC)
	    fputs(" outside", _stderr);
	else {
	    switch (key->margin) {
	    case GPKEY_TMARGIN:
		fputs(" tmargin", _stderr);
		break;
	    case GPKEY_BMARGIN:
		fputs(" bmargin", _stderr);
		break;
	    case GPKEY_LMARGIN:
		fputs(" lmargin", _stderr);
		break;
	    case GPKEY_RMARGIN:
		fputs(" rmargin", _stderr);
		break;
	    }
	}
	fputs("\n", _stderr);
...
\tkey is %s justified, %sreversed, %sinverted, %senhanced and ",
	...
	fprintf(_stderr, "boxed\n\twith ");
	save_linetype(_stderr, &(key->box), FALSE);
	fputc('\n', _stderr);
    } else
	fprintf(_stderr, "not boxed\n");

    if (key->front) {
	fprintf(_stderr, "\tkey box is opaque");
	if (key->fillcolor.lt != LT_BACKGROUND)
	    save_pm3dcolor(_stderr, &key->fillcolor);
	fprintf(_stderr," \n");
    }
...
\tcurves are%s automatically titled %s\n",
    ...
    fputs("\tmaximum number of columns is ", _stderr);
    if (key->maxcols > 0)
	fprintf(_stderr, "%d for horizontal alignment\n", key->maxcols);
    else
	fputs("calculated automatically\n", _stderr);
    fputs("\tmaximum number of rows is ", _stderr);
    if (key->maxrows > 0)
	fprintf(_stderr, "%d for vertical alignment\n", key->maxrows);
    else
	fputs("calculated automatically\n", _stderr);
	...
	fputs("\t ", _stderr);
	save_textcolor(_stderr, &(key->textcolor));
	fputs("\n", _stderr);
    }
...
{
    fprintf(_stderr,"(");
    save_position(_stderr, pos, ndim, FALSE);
    fprintf(_stderr,")");
}
...
{
    if (axis->log) {
	fprintf(_stderr, " %s", axis_name(axis->index));
	if (axis->base != 10.)
	    fprintf(_stderr, " (base %g)", axis->base);
	return 1;
    }
    return 0;
}
...
show_offsets()
{
    ...
    save_offsets(_stderr,"\toffsets are");
}
...
show_margin()
{
	...
	fprintf(_stderr, "\tlmargin is set to screen %g\n", lmargin.x);
    else if (lmargin.x >= 0)
	fprintf(_stderr, "\tlmargin is set to %g\n", lmargin.x);
    else
	fputs("\tlmargin is computed automatically\n", _stderr);

    if (rmargin.scalex == screen)
	fprintf(_stderr, "\trmargin is set to screen %g\n", rmargin.x);
    else if (rmargin.x >= 0)
	fprintf(_stderr, "\trmargin is set to %g\n", rmargin.x);
    else
	fputs("\trmargin is computed automatically\n", _stderr);

    if (bmargin.scalex == screen)
	fprintf(_stderr, "\tbmargin is set to screen %g\n", bmargin.x);
    else if (bmargin.x >= 0)
	fprintf(_stderr, "\tbmargin is set to %g\n", bmargin.x);
    else
	fputs("\tbmargin is computed automatically\n", _stderr);

    if (tmargin.scalex == screen)
	fprintf(_stderr, "\ttmargin is set to screen %g\n", tmargin.x);
    else if (tmargin.x >= 0)
	fprintf(_stderr, "\ttmargin is set to %g\n", tmargin.x);
    else
...
show_output()
{
    ...
    if (outstr)
	fprintf(_stderr, "\toutput is sent to '%s'\n", outstr);
    else
...
show_print()
{
	...
	fprintf(_stderr, "\tprint output is sent to '%s'\n", print_show_output());
    else
	fprintf(_stderr, "\tprint output is saved to datablock %s\n", print_show_output());
}
...
show_psdir()
{
    ...
    fprintf(_stderr, "\tdirectory from 'set psdir': ");
    fprintf(_stderr, "%s\n", PS_psdir ? PS_psdir : "none");
    fprintf(_stderr, "\tenvironment variable GNUPLOT_PS_DIR: ");
    fprintf(_stderr, "%s\n", getenv("GNUPLOT_PS_DIR") ? getenv("GNUPLOT_PS_DIR") : "none");
#ifdef GNUPLOT_PS_DIR
    fprintf(_stderr, "\tdefault system directory \"%s\"\n", GNUPLOT_PS_DIR);
#else
    fprintf(_stderr, "\tfall through to built-in defaults\n");
#endif
}
...
show_overflow()
{
    fprintf(_stderr, "\t64-bit integer overflow %s\n",
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tparametric is %s\n", (parametric) ? "ON" : "OFF");
}
...
{
    int i;
    fprintf(_stderr,"\t  * there are %i available rgb color mapping formulae:",
	...
	if (i % 3 == 0)
	    fputs("\n\t    ", _stderr);
	fprintf(_stderr, "%2i: %-15s",i,ps_math_color_formulae[2*i+1]);
	i++;
    }
    fputs("\n", _stderr);
...
#define rgb_distance(r,g,b) ((r)*(r) + (g)*(g) + (b)*(b))
    ...
    fprintf(_stderr, "\tThe best match of the current palette corresponds to\n\t    set palette rgbformulae %i,%i,%i\n", rMin, gMin, bMin);
#undef rgb_distance
    for (i = 0; i < 2*maxFormula+1; i++)
	free(formulae[i]);
    free(formulae);
...
show_palette_palette()
{
    ...
    if (!i)
	fprintf(_stderr," saved to \"%s\".", print_out_name);
    else
	fprintf(_stderr, ".\n");
...
show_colornames(const struct gen_table *tbl)
{
	...
	fprintf( _stderr, "\n  %-18s ", tbl->key );
	fprintf(_stderr, "#%02x%02x%02x = %3i %3i %3i", r,g,b, r,g,b);
	++tbl;
	++i;
    }
    fputs( "\n", _stderr );
...
show_palette_colornames()
{
    fprintf(_stderr, "\tThere are %d predefined color names:", num_predefined_colors);
...
show_palette()
{
	...
	switch( sm_palette.colorMode ) {
	  ...
	  case SMPAL_COLOR_MODE_RGB:
	    fprintf(_stderr,"\trgb color mapping by rgbformulae are %i,%i,%i\n",
	...
	fputs("\tallocating ", _stderr);
	if (sm_palette.use_maxcolors)
	    fprintf(_stderr,"MAX %i",sm_palette.use_maxcolors);
	else
	    fputs("ALL remaining", _stderr);
	fputs(" color positions for discrete palette terminals\n", _stderr);
	fputs( "\tColor-Model: ", _stderr );
...
show_colorbox()
{
    c_token++;
    if (color_box.border) {
	fputs("\tcolor box with border, ", _stderr);
	if (color_box.border_lt_tag >= 0)
	    fprintf(_stderr,"line type %d is ", color_box.border_lt_tag);
	else
	    fputs("DEFAULT line type is ", _stderr);
    } else {
	fputs("\tcolor box without border is ", _stderr);
    }
	...
	fprintf(_stderr,"\tcolor gradient is vertical %s\n",
	color_box.invert ? " (inverted)" : "");
    else
	fprintf(_stderr,"\tcolor gradient is horizontal\n");
}
...
{
    c_token++;
    fprintf(_stderr,"\tpm3d style is %s\n", PM3D_IMPLICIT == pm3d.implicit ? "implicit (pm3d draw for all surfaces)" : "explicit (draw pm3d surface according to style)");
    fputs("\tpm3d plotted at ", _stderr);
    { int i=0;
	for ( ; pm3d.where[i]; i++ ) {
	    if (i>0) fputs(", then ", _stderr);
	    switch (pm3d.where[i]) {
		case PM3D_AT_BASE: fputs("BOTTOM", _stderr); break;
		case PM3D_AT_SURFACE: fputs("SURFACE", _stderr); break;
		case PM3D_AT_TOP: fputs("TOP", _stderr); break;
	    }
	}
	fputs("\n", _stderr);
    }
    if (pm3d.direction == PM3D_DEPTH) {
	fprintf(_stderr,"\ttrue depth ordering\n");
    } else if (pm3d.direction != PM3D_SCANS_AUTOMATIC) {
	fprintf(_stderr,"\ttaking scans in %s direction\n",
	    pm3d.direction == PM3D_SCANS_FORWARD ? "FORWARD" : "BACKWARD");
    } else {
	fputs("\ttaking scans direction automatically\n", _stderr);
    }
    fputs("\tsubsequent scans with different nb of pts are ", _stderr);
    if (pm3d.flush == PM3D_FLUSH_CENTER) fputs("CENTERED\n", _stderr);
    else fprintf(_stderr,"flushed from %s\n",
	pm3d.flush == PM3D_FLUSH_BEGIN ? "BEGIN" : "END");
    fprintf(_stderr,"\tflushing triangles are %sdrawn\n",
	pm3d.ftriangles ? "" : "not ");
    fputs("\tclipping: ", _stderr);
	...
	fprintf(_stderr,"\tpm3d quadrangles will have no border\n");
    } else {
	fprintf(_stderr,"\tpm3d quadrangle borders will default to ");
	save_linetype(_stderr, &(pm3d.border), FALSE);
	fprintf(_stderr,"\n");
    }
    if (pm3d_shade.strength > 0) {
	fprintf(_stderr,"\tlighting primary component %g specular component %g",
		pm3d_shade.strength, pm3d_shade.spec);
	fprintf(_stderr," second spot contribution %g\n",
		pm3d_shade.spec2);
    }
    fprintf(_stderr,"\tsteps for bilinear interpolation: %d,%d\n",
	 pm3d.interp_i, pm3d.interp_j);
    fprintf(_stderr,"\tquadrangle color according to ");
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tpointsize is %g\n", pointsize);
}
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tpointintervalbox is %g\n", pointintervalbox);
}
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tRGB image color components are in range [0:%g]\n", rgbmax);
}
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tnominal character encoding is %s\n", encoding_names[encoding]);
#ifdef HAVE_LOCALE_H
    fprintf(_stderr, "\thowever LC_CTYPE in current locale is %s\n", setlocale(LC_CTYPE,NULL));
#endif
}
...
show_decimalsign()
{
    ...
    set_numeric_locale();
    fprintf(_stderr, "\tdecimalsign for input is  %s \n", get_decimal_locale());
    reset_numeric_locale();

    if (decimalsign!=NULL)
        fprintf(_stderr, "\tdecimalsign for output is %s \n", decimalsign);
    else
        fprintf(_stderr, "\tdecimalsign for output has default value (normally '.')\n");

    fprintf(_stderr, "\tdegree sign for output is %s \n", degree_sign);
}
...
show_micro()
{
    ...
    fprintf(_stderr, "\tmicro character for output is %s \n",
...
show_minus_sign()
{
    ...
    if (use_minus_sign && minus_sign)
        fprintf(_stderr, "\tminus sign for output is %s \n", minus_sign);
    else
        fprintf(_stderr, "\tno special minus sign\n");
}
...
show_fit()
{
	...
	case QUIET:
	    fprintf(_stderr, "\tfit will not output results to console.\n");
	    break;
	case RESULTS:
	    fprintf(_stderr, "\tfit will only print final results to console and log-file.\n");
	    break;
	case BRIEF:
	    fprintf(_stderr, "\tfit will output brief results to console and log-file.\n");
	    if (fit_wrap)
		fprintf(_stderr, "\toutput of long lines will be wrapped at column %i.\n", fit_wrap);
	    break;
	case VERBOSE:
	    fprintf(_stderr, "\tfit will output verbose results to console and log-file.\n");
	    break;
    }

    fprintf(_stderr, "\tfit can handle up to %d independent variables\n",
	    GPMIN(MAX_NUM_VAR,MAXDATACOLS-2));

    fprintf(_stderr, "\tfit will%s prescale parameters by their initial values\n",
	    fit_prescale ? "" : " not");

    fprintf(_stderr, "\tfit will%s place parameter errors in variables\n",
	    fit_errorvariables ? "" : " not");
    fprintf(_stderr, "\tfit will%s place covariances in variables\n",
    ...
    if (fit_suppress_log) {
	fprintf(_stderr,"\tfit will not create a log file\n");
    ...
    fprintf(_stderr, "\tfits will be considered to have converged if  delta chisq < chisq * %g",
	((d > 0.) && (d < 1.)) ? d : DEF_FIT_LIMIT);
    if (epsilon_abs > 0.)
	fprintf(_stderr, " + %g", epsilon_abs);
    fprintf(_stderr, "\n");

    v = get_udv_by_name((char *)FITMAXITER);
    if ((v != NULL) && (v->udv_value.type != NOTDEFINED) && (real(&(v->udv_value)) > 0))
	fprintf(_stderr, "\tfit will stop after a maximum of %i iterations\n",
	        (int)real(&(v->udv_value)));
    else
	fprintf(_stderr, "\tfit has no limit in the number of iterations\n");

    v = get_udv_by_name((char *)FITSTARTLAMBDA);
    d = ((v != NULL) && (v->udv_value.type != NOTDEFINED)) ? real(&(v->udv_value)) : -1.0;
    if (d > 0.)
	fprintf(_stderr, "\tfit will start with lambda = %g\n", d);

    v = get_udv_by_name((char *)FITLAMBDAFACTOR);
    d = ((v != NULL) && (v->udv_value.type != NOTDEFINED)) ? real(&(v->udv_value)) : -1.0;
    if (d > 0.)
	fprintf(_stderr, "\tfit will change lambda by a factor of %g\n", d);

    if (fit_v4compatible)
	fprintf(_stderr, "\tfit command syntax is backwards compatible to version 4\n");
    else
	fprintf(_stderr, "\tfit will default to `unitweights` if no `error`keyword is given on the command line.\n");
    fprintf(_stderr, "\tfit can run the following command when interrupted:\n\t\t'%s'\n", getfitscript());
    v = get_udv_by_name("GPVAL_LAST_FIT");
    if (v != NULL && v->udv_value.type != NOTDEFINED)
	fprintf(_stderr, "\tlast fit command was: %s\n", v->udv_value.v.string_val);
}
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tsampling rate is %d, %d\n", samples_1, samples_2);
}
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tiso sampling rate is %d, %d\n",
...
{
    SHOW_ALL_NL;
    fputs("\tview is ", _stderr);
    if (splot_map == TRUE) {
	fprintf(_stderr,"map scale %g\n", mapview_scale);
	return;
    } else if (xz_projection) {
	fprintf(_stderr,"xz projection\n");
    } else if (yz_projection) {
	fprintf(_stderr,"yz projection\n");
    } else {
	fprintf(_stderr, "%g rot_x, %g rot_z, %g scale, %g scale_z\n",
		surface_rot_x, surface_rot_z, surface_scale, surface_zscale);
    }
    fprintf(_stderr,"\t\t%s axes are %s\n",
		aspect_ratio_3D == 2 ? "x/y" : aspect_ratio_3D == 3 ? "x/y/z" : "",
		aspect_ratio_3D >= 2 ? "on the same scale" : "independently scaled");
    fprintf(_stderr, "\t\t azimuth %g\n", azimuth);
}
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tsurface is %sdrawn %s\n",
...
show_hidden3d()
{
    ...
    fprintf(_stderr, "\thidden surface is %s\n",
...
show_increment()
{
    fprintf(_stderr,"\tPlot lines increment over ");
    if (prefer_line_styles)
	fprintf(_stderr, "user-defined line styles rather than default line types\n");
    else
	fprintf(_stderr, "default linetypes\n");
}

static void
show_histogram()
{
    fprintf(_stderr,"\tHistogram style is ");
    save_histogram_opts(_stderr);
}

static void
show_textbox()
{
    save_style_textbox(_stderr);
}
...
show_history()
{
    ...
    fprintf(_stderr, "\tThis copy of gnuplot was not built to use a command history file\n");
#endif
    fprintf(_stderr, "\t history size %d%s,  %s,  %s\n",
...
show_size()
{
    ...
    fprintf(_stderr, "\tsize is scaled by %g,%g\n", xsize, ysize);
    if (aspect_ratio > 0)
	fprintf(_stderr, "\tTry to set aspect ratio to %g:1.0\n", aspect_ratio);
    else if (aspect_ratio == 0)
	fputs("\tNo attempt to control aspect ratio\n", _stderr);
    else
	fprintf(_stderr, "\tTry to set LOCKED aspect ratio to %g:1.0\n",
		-aspect_ratio);
}
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\torigin is set to %g,%g\n", xoffset, yoffset);
}
...
show_term()
{
    ...
    if (term)
	fprintf(_stderr, "   terminal type is %s %s\n",
    ...
    TBOOLEAN showcb)
{
    ...
    fprintf(_stderr, "\ttics are in %s of plot\n", (grid_tics_in_front) ? "front" : "back");
    ...
    fprintf(_stderr,"\tScales for user tic levels 2-%d are: ",MAX_TICLEVEL-1);
    for (i=2; i<MAX_TICLEVEL; i++)
	fprintf(_stderr, " %g%c", ticscale[i], i<MAX_TICLEVEL-1 ? ',' : '\n');
...
show_mtics(struct axis *axis)
{
    ...
    case MINI_OFF:
	fprintf(_stderr, "\tminor %stics are off\n", name);
	break;
    case MINI_DEFAULT:
	fprintf(_stderr, "\
...
\tminor %stics are computed automatically for log scales\n",
	...
	break;
    case MINI_AUTO:
	fprintf(_stderr, "\tminor %stics are computed automatically\n", name);
	break;
    case MINI_USER:
	fprintf(_stderr, "\
...
show_timestamp()
{
    ...
    fprintf(_stderr, "\twritten in %s corner\n",
...
show_range(AXIS_INDEX axis)
{
	...
	fprintf(_stderr, "\tset %sdata time\n", axis_name(axis));
    fprintf(_stderr,"\t");
    save_prange(_stderr, axis_array + axis);
}
...
{
    if (label) {
	fprintf(_stderr, "\t%s%s is \"%s\", offset at ", name, suffix,
	    label->text ? conv_text(label->text) : "");
	show_position(&label->offset, 3);
	fprintf(_stderr, label->pos == LEFT ? " left justified"
    ...
    if (label->font)
	fprintf(_stderr, ", using font \"%s\"", conv_text(label->font));

    if (label->tag == ROTATE_IN_3D_LABEL_TAG)
	fprintf(_stderr, ", parallel to axis in 3D plots");
    else if (label->rotate)
	fprintf(_stderr, ", rotated by %d degrees in 2D plots", label->rotate);

    if (label->textcolor.type)
	save_textcolor(_stderr, &label->textcolor);
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tDefault format for reading time data is \"%s\"\n",
	timefmt);
}
...
show_link()
{
    if (END_OF_COMMAND || almost_equals(c_token,"x$2"))
	    save_link(_stderr, axis_array + SECOND_X_AXIS);
    if (END_OF_COMMAND || almost_equals(c_token,"y$2"))
	    save_link(_stderr, axis_array + SECOND_Y_AXIS);
    if (!END_OF_COMMAND)
	c_token++;
}
...
show_nonlinear()
{
	...
	save_nonlinear(_stderr, &axis_array[axis]);
}
...
show_fontpath()
{
    ...
    SHOW_ALL_NL;
    fprintf(_stderr, "\tdirectory from 'set fontpath': %s\n",
	PS_fontpath ? PS_fontpath : "none");
    fprintf(_stderr, "\tenvironmental variable GNUPLOT_FONTPATH: %s\n",
...
show_datafile()
{
	...
	if (missing_val == NULL)
	    ...
	    fprintf(_stderr,"\tall NaN (not-a-number) values will be treated as missing data\n");
	else
	    fprintf(_stderr, "\t\"%s\" in datafile is interpreted as missing value\n",
		missing_val);
    }
    if (END_OF_COMMAND || almost_equals(c_token,"sep$arators")) {
	if (df_separators)
	    fprintf(_stderr, "\tdatafile fields separated by any of %d characters \"%s\"\n", (int)strlen(df_separators), df_separators);
	else
	    fprintf(_stderr, "\tdatafile fields separated by whitespace\n");
    }
    if (END_OF_COMMAND || almost_equals(c_token,"com$mentschars")) {
	fprintf(_stderr, "\tComments chars are \"%s\"\n", df_commentschars);
    }
    if (END_OF_COMMAND || almost_equals(c_token,"columnhead$ers")) {
	if (df_columnheaders)
	    fprintf(_stderr, "\tFirst line is always treated as headers rather than data\n");
	else
	    fprintf(_stderr, "\tFirst line is treated as headers only if accessed explicitly\n");
    }
...
show_table()
{
    ...
    SHOW_ALL_NL;
    if (table_mode)
	fprintf(_stderr, "\ttable mode is on, field separator %s\n",
		foo[0] == '\t' ? "tab" :
		foo[0] == ',' ? "comma" :
		foo[0] == ' ' ? "space" :
		foo);
    else
	fprintf(_stderr, "\ttable mode is off\n");
}
...
show_mouse()
{
    ...
    SHOW_ALL_NL;
    if (mouse_setting.on) {
	fprintf(_stderr, "\tmouse is on\n");
	if (mouse_setting.annotate_zoom_box) {
	    fprintf(_stderr, "\tzoom coordinates will be drawn\n");
	} else {
	    fprintf(_stderr, "\tno zoom coordinates will be drawn\n");
	}
	if (mouse_setting.polardistance) {
	    fprintf(_stderr, "\tdistance to ruler will be show in polar coordinates\n");
	} else {
	    fprintf(_stderr, "\tno polar distance to ruler will be shown\n");
	}
	if (mouse_setting.doubleclick > 0) {
	    fprintf(_stderr, "\tdouble click resolution is %d ms\n",
		mouse_setting.doubleclick);
	} else {
	    fprintf(_stderr, "\tdouble click resolution is off\n");
	}
	if (mouse_mode == MOUSE_COORDINATES_FUNCTION)
	    fprintf(_stderr, "\tcoordinate readout via mouseformat function %s\n",
		mouse_readout_function.definition);
	else if (mouse_mode == MOUSE_COORDINATES_ALT)
	    fprintf(_stderr, "\tcoordinate readout via mouseformat '%s'\n",
		mouse_alt_string);
	else
	    fprintf(_stderr, "\tcoordinate readout via mouseformat %d\n",
		(int)mouse_mode);
	fprintf(_stderr, "\tformat for individual coordinates is '%s'\n",
	    mouse_setting.fmt);
	if (mouse_setting.label) {
	    fprintf(_stderr, "\tButton 2 draws persistent labels with options \"%s\"\n",
		mouse_setting.labelopts);
	} else {
	    fprintf(_stderr, "\tButton 2 draws temporary labels\n");
	}
	fprintf(_stderr, "\tzoom factors are x: %g   y: %g\n",
		mouse_setting.xmzoom_factor, mouse_setting.ymzoom_factor);
	fprintf(_stderr, "\tzoomjump is %s\n",
	    mouse_setting.warp_pointer ? "on" : "off");
	fprintf(_stderr, "\tcommunication commands will %sbe shown\n",
	    mouse_setting.verbose ? "" : "not ");
    } else {
	fprintf(_stderr, "\tmouse is off\n");
    }
#else  /* USE_MOUSE */
...
{
    SHOW_ALL_NL;
    fprintf(_stderr, "\tlast plot command was: %s\n", replot_line);
}
...
show_variables()
{
	...
	if (udv->udv_value.type == NOTDEFINED) {
	    ...
	    fprintf(_stderr, "\t%-*s ", len, udv->udv_name);
	    fputs("= ", _stderr);
	    disp_value(_stderr, &(udv->udv_value), TRUE);
	    (void) putc('\n', _stderr);
	}
	udv = udv->next_udv;
    }
...
show_linestyle(int tag)
{
	...
	if (tag == 0 || tag == this_linestyle->tag) {
	    showed = TRUE;
	    fprintf(_stderr, "\tlinestyle %d, ", this_linestyle->tag);
	    save_linetype(_stderr, &(this_linestyle->lp_properties), TRUE);
	    fputc('\n', _stderr);
	}
    }
    if (tag > 0 && !showed)
...
show_linetype(struct linestyle_def *listhead, int tag)
{
	...
	if (tag == 0 || tag == this_linestyle->tag) {
	    showed = TRUE;
	    fprintf(_stderr, "\tlinetype %d, ", this_linestyle->tag);
	    save_linetype(_stderr, &(this_linestyle->lp_properties), TRUE);
	    fputc('\n', _stderr);
	}
    }
	...
	fprintf(_stderr, "\tLinetypes repeat every %d unless explicitly defined\n",
		recycle_count);
}
...
show_arrowstyle(int tag)
{
	...
	if (tag == 0 || tag == this_arrowstyle->tag) {
	    showed = TRUE;
	    fprintf(_stderr, "\tarrowstyle %d, ", this_arrowstyle->tag);
	    fflush(_stderr);

	    fprintf(_stderr, "\t %s %s",
		    arrow_head_names[this_arrowstyle->arrow_properties.head],
		    this_arrowstyle->arrow_properties.layer ? "front" : "back");
	    save_linetype(_stderr, &(this_arrowstyle->arrow_properties.lp_properties), FALSE);
	    fputc('\n', _stderr);

	    if (this_arrowstyle->arrow_properties.head > 0) {
		fprintf(_stderr, "\t  arrow heads: %s, ",
		...
		if (this_arrowstyle->arrow_properties.head_length > 0) {
			...
			{"(first x axis) ", "(second x axis) ",
			 ...
			 "(character units) "};
		    fprintf(_stderr," length %s%g, angle %g deg",
		    ...
		    fprintf(_stderr," (default length and angles)");
		}
...
show_ticdefp(struct axis *this_axis)
{
    ...
    fprintf(_stderr, "\t%s-axis tics are %s, \
\tmajor ticscale is %g and minor ticscale is %g\n",
    ...
    fprintf(_stderr, "\t%s-axis tics:\t", axis_name(this_axis->index));
    switch (this_axis->ticmode & TICS_MASK) {
    case NO_TICS:
	fputs("OFF\n", _stderr);
	return;
    case TICS_ON_AXIS:
	fputs("on axis", _stderr);
	if (this_axis->ticmode & TICS_MIRROR)
	    fprintf(_stderr, " and mirrored %s", (this_axis->tic_in ? "OUT" : "IN"));
	break;
    case TICS_ON_BORDER:
	fputs("on border", _stderr);
	if (this_axis->ticmode & TICS_MIRROR)
	    fputs(" and mirrored on opposite border", _stderr);
	break;
    }
    ...
    if (this_axis->manual_justify) {
    	...
    	case LEFT:{
		fputs("left justified, ", _stderr);
		break;
	    }
    	case RIGHT:{
		fputs("right justified, ", _stderr);
		break;
	    }
    	case CENTRE:{
		fputs("center justified, ", _stderr);
		break;
	    }
    	}
    } else
        fputs("justified automatically, ", _stderr);
    fprintf(_stderr, "format \"%s\"", ticfmt);
    fprintf(_stderr, "%s",
	...
	fprintf(_stderr," rotated");
	fprintf(_stderr," by %d",this_axis->tic_rotate);
	fputs(" in 2D mode, terminal permitting,\n\t", _stderr);
    } else
	fputs(" and are not rotated,\n\t", _stderr);
    fputs("    offset ",_stderr);
    ...
    case TIC_SERIES:{
	    fputs("  series", _stderr);
	    if (this_axis->ticdef.def.series.start != -VERYLARGE) {
		fputs(" from ", _stderr);
		save_num_or_time_input(_stderr, this_axis->ticdef.def.series.start, this_axis);
	    }
	    fprintf(_stderr, " by %g%s", this_axis->ticdef.def.series.incr,
		    this_axis->datatype == DT_TIMEDATE ? " secs" : "");
	    if (this_axis->ticdef.def.series.end != VERYLARGE) {
		fputs(" until ", _stderr);
		save_num_or_time_input(_stderr, this_axis->ticdef.def.series.end, this_axis);
	    }
	    putc('\n', _stderr);
	    break;
	}
	...
	fputs("\t  explicit list (", _stderr);
	for (t = this_axis->ticdef.def.user; t != NULL; t = t->next) {
	    if (t->label)
		fprintf(_stderr, "\"%s\" ", conv_text(t->label));
	    save_num_or_time_input(_stderr, t->position, this_axis);
	    if (t->level)
		fprintf(_stderr," %d",t->level);
	    if (t->next)
		fputs(", ", _stderr);
	}
	fputs(")\n", _stderr);
    ...
    if (this_axis->ticdef.textcolor.type != TC_DEFAULT) {
        fputs("\t ", _stderr);
	save_textcolor(_stderr, &this_axis->ticdef.textcolor);
        fputs("\n", _stderr);
    }
