...
");
	...
	c_token++;
	fprintf(_stderr, "\nCompile options:\n%s", compile_options);
	fprintf(_stderr, "    MAX_PARALLEL_AXES=%d\n\n", MAX_PARALLEL_AXES);
...
show_paxis()
{
	...
	save_prange(_stderr, &parallel_axis[p-1]);
...
\tkey is %s justified, %sreversed, %sinverted, %senhanced and ",
    ...
    if (key->front)
	fprintf(_stderr, "\tkey box is opaque and drawn in front of the graph\n");
...
show_mouse()
{
	...
	fprintf(_stderr, "\tformatting numbers with \"%s\"\n",
	    mouse_setting.fmt);
	fprintf(_stderr, "\tformat for Button 2 is %d\n", (int) mouse_mode);
	if (mouse_alt_string) {
	    fprintf(_stderr, "\talternative format for Button 2 is '%s'\n",
		mouse_alt_string);
	}
